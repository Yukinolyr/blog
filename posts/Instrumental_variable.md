...
#### IV估计：基本设定
考虑基本模型：$$Y_{i}=\beta _{0}+\beta _{1}X_{i}+u_{i}$$，且$$Cov(X_{i},u_{i}) \neq 0$$。
若找到满足$$Cov(Z_{i}, X_{i}) \neq0$$且$$Cov(Z_{i}, u_{i})=0$$的工具变量$Z_{i}$，则IV估计量为：$$\hat{\beta}_{IV}=\frac{Cov\left(Z_{i}, Y_{i}\right)}{Cov\left(Z_{i}, X_{i}\right)}$$
...
#### 意向治疗效应（ITT）的分解
意向治疗效应（ITT effect）可分解为各子群体ITT效应的组合：$$ITT=ITT_{c}× Pr(依从者)+ITT_{a}× Pr(总是接受者)+ITT_{n}× Pr(从不接受者)+ITT_{d}× Pr(反向遵从者)$$
其中：
...
核心变量（县层面面板数据）：
- \(asthma_{ct}\)：儿童哮喘住院率（每千人入院数）
- \(so2_{ct}\)：\(SO_2\)浓度（\(\mu g/m^3\)）
- \(plant_{ct}\)：燃煤电厂存在虚拟变量
- \(income_{ct}\)：人均收入（USD）
- \(unemp_{ct}\)：失业率（%）
...
###### 解析
\(SO_2\)浓度升高会增加儿童哮喘住院率（真实效应为正）。我们会遗漏关键变量：污染缓解行为（mitigation behavior），包括使用空气净化器、佩戴口罩、减少户外活动、地方防控措施等。

- 污染越严重的地区，居民的缓解行为越强（\(so2_{ct}\)与缓解行为正相关）；
- 缓解行为会降低哮喘住院率（缓解行为与\(asthma_{ct}\)负相关）；
- 这导致OLS回归的误差项\(u_{ct}\)包含缓解行为的影响，且满足$$Cov(so2_{ct}, u_{ct}) < 0$$；
- 负向遗漏变量偏差会“稀释”\(SO_2\)对哮喘的正向效应，最终使OLS系数（0.062）小于IV估计的真实因果效应（0.185）。
...
##### Q3
What empirical strategy addresses endogeneity from time-invariant county differences?

###### 答案
采用**县固定效应（county fixed effects）** 策略，若\(so2_{ct}\)仍存在内生性，需结合固定效应与2SLS（固定效应2SLS）。

###### 解析
县层面存在不随时间变化的未观测特征（如地理环境、长期医疗资源、居民遗传背景等），这些特征既影响\(SO_2\)浓度（如偏远县污染更低），又影响哮喘住院率（如医疗资源差的县住院率可能偏高），导致内生性。通过控制县固定效应\(\alpha_c\)，剥离时间不变的未观测特征的影响，就能使估计聚焦于“县内随时间变化的\(SO_2\)浓度”对哮喘住院率的效应。
对应的回归方程：
   $$asthma_{ct} = \beta so2_{ct} + \gamma income_{ct} + \delta unemp_{ct} + \alpha_c + \varepsilon_{ct}$$
   其中，\(\alpha_c\)为县固定效应，\(\varepsilon_{ct}\)为随时间和县域变化的误差项。
...
###### 解析
定义“上风向电厂”：
给定主导风向，若至少有一家燃煤电厂位于县\(c\)的上风向区域，则UpwindPlant\(_{ct}=1\)，否则为0。其优势具体如下：
- 县内电厂的污染可能被本地防控措施（如废气处理设备）削弱，对本地\(SO_2\)浓度的影响不稳定；
- 上风向电厂排放的污染物会随主导风直接扩散至目标县，对本地\(SO_2\)浓度的贡献更直接、稳定，能强化第一阶段回归的相关性（使\(Cov(UpwindPlant_{ct}, so2_{ct})\)更大）。
- 县内电厂的建设和运营可能与本地经济政策、产业规划高度相关（如为促进就业引入电厂），这些因素可能直接影响儿童哮喘住院率（如经济发达县医疗资源更好），导致\(plant_{ct}\)与误差项相关，违反排他性；
